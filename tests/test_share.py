...
ck = result.get("check") or {}
check(ck.get("success") is True and ck.get("files") == 3 and ck.get("dirs") == 3, "转存结果含内容检测统计（一次遍历完成检测+转存）")
check(ck.get("total_size") == 3010 and len(ck.get("root_items") or []) == 2, "检测统计含总大小与根目录项")
...
check(task.start_run() is False, "start_run 与 start_sync 共用防重入锁")
...
check((st["last_result"].get("check") or {}).get("success") is True, "上次结果含分享检测统计")
...
check((result.get("check") or {}).get("success") is True, "转存失败不影响分享检测统计")
